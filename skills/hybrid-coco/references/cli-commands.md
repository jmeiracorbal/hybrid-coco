...
`.hybrid-coco/config.toml` is written by `hc init` / `hc index` if missing. All three keys are required:
...
`hc reset` keeps `config.toml`. `hc doctor` fails if the file is missing or invalid.
