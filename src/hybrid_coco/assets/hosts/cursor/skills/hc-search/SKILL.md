---
name: hc-search
description: Query the hybrid-coco index from Cursor — choose hc_symbol, hc_search, or hc_file_context. Does not reindex.
user-invokable: true
args:
  - name: query
...
Do not open files with Cursor `Read` or `Grep` to answer this query. Use `hc_*` first.
...
2. Hand off to `/hc-init` (`hc init . --host cursor`) — do not full-reindex from this skill as a first step

## Present results

Group by file when useful. For each hit show: kind, path, line range, signature, docstring if any.

After a hit, if the body is needed: `hc_snippet(path, line_start, line_end)` from the symbol range — not Cursor `Read` of the whole file.
