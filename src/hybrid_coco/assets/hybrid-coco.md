...
- `hc index` / `hc update` / `hc query` / `hc symbol` / `hc file-context` / `hc snippet` / `hc structure` / `hc embed` / `hc semantic`
- MCP tools: `hc_search`, `hc_symbol`, `hc_file_context`, `hc_snippet`, `hc_structure`, `hc_semantic`, `hc_status`
...
| `hc_semantic("query", path?, lang?, offset?, limit?)` | Optional nearest-neighbour search; only if `hc embed` has already been run |
